...
class TestPadBatches():
    ...
    def test_should_correctly_pad_data(self):
        assert pad_batches(TRUNCATED_SCRAMBLED_BATCHES_4, 4, PAD) == PADDED_SCRAMBLED_BATCHES_4
        assert pad_batches(TRUNCATED_SORTED_BATCHES_4, 4, PAD) == PADDED_SORTED_BATCHES_4

class TestMakeBatches():
    def test_should_do_nothing_when_data_is_empty(self):
        assert make_batches([], [], 2, 4, PAD) == ([], [], [])
    ...
    def test_should_correctly_create_batches(self):
        ...
        for batch in y_batches:
            for example in batch:
                assert len(example) <= 4
        assert x_batches == PADDED_SORTED_BATCHES_4
        assert y_batches == TRUNCATED_SORTED_BATCHES_4
